...
<div class = "max-w-7xl mx-auto sm:px-6 lg:px-8 pt-4">
    ...
    @elseif(Route::is('journals.notpublicated'))
        <form action = "{{ route('deleteall') }}" method = "post">@csrf<input  class = "bg-red-500 appearance-none rounded text-white leading-tight focus:outline-none focus:bg-gray-500 py-2 px-4 hover:bg-black  transition duration-300 ease-in-out" onclick = "return shure()" value = "Удалить все черновики" type = "submit"></form>
    @elseif(Route::is('journals.authors' ))   
...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mb-4">    
            ...
            <div class="relative float-left mb-4 flex flex-col justify-between justify-between h-96 overflow-hidden shadow sm:rounded-lg journalcard w-full" style = "@if($journal->img != NULL) background: url(/storage/app/public/images/{{ $journal->img }}) no-repeat; background-color:#ccc; background-size:38rem; @else background: url(/public/images/default.png) no-repeat; background-size:cover; @endif height:40rem;">  
                <div class = "flex flex-col bg-black bg-opacity-60 p-4 text-white">
                    ...
                    @endif
                    <h1 class = "text-2xl font-bold pb-1">{{ $journal->name }}</h1>
                    <div class = "flex flex-row pb-1"><b class = "pr-2">Описание:</b> <p class = "">{{ $journal->desc }}</p></div>
                    <div class = "pb-1 text-sm">
                        <b class = "pr-2 float-left">Авторы:</b>
                    @foreach($readyauthors as $readyauthor)
                        @if($readyauthor->journal_id == $journal->id)                            
                            <p class = "flex flex-row whitespace-pre float-left">{{ $readyauthor->surname }} {{ $readyauthor->name }}  {{ $readyauthor->patronymic }}&nbsp·&nbsp</p>
                        @endif
                    @endforeach                        
                    </div>
                </div>                    
                ...
                <div class = "w-full flex flex-row p-2 justify-between bg-white bg-opacity-80 p-4">
                    ...
                    <a href = "{{ route('journal.editpage',[$journal->id]) }}"><img src = "/public/images/edit.png" class ="cursor-pointer px-2 w-10 h-6"></a>
                    <form action = "{{ route('journal.delete') }}" method = "post" name = "deleteform">
                        @csrf
                        <input class = "hidden" value = "{{ $journal->id}}" name = "id">
                        <input class = "px-2 w-10 h-6" type = "image" src = "/public/images/delete.png" onclick = "return shure()" name = "journalpage">
                    </form> 
                    </div>
                </div>
